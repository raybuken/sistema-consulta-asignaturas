...
import { updateDispatch } from '../../helpers/dispatch/dispatchHelpers';
import { editDispatchSchema } from '../../validations/dispatch';

function EditDispatchForm({id, dispatchData}) {
  const toast = useToast()
  const initialValues = {
    name: dispatchData.name,
    code: dispatchData.code,
    location: dispatchData.location,
    active: dispatchData.active
  }
      ...
      .then(data => {
        if(!data.ok){
          ...
          toast({
            ...
            description: "Campos del despacho actualizado",
          })
        }
  ...
  return (
    <Box rounded='md' padding='5' boxShadow='lg' role='group' minH={400} w={"100%"}>
      <VStack>
        ...
        <Heading as='h1' textAlign='center'>Editar Despacho</Heading>

      </VStack>
      <Formik
        initialValues={initialValues}
        onSubmit={handleSubmit}
        validationSchema={editDispatchSchema}
      >
        {({handleSubmit, handleChange, handleBlur, isSubmitting, values, touched, errors}) => (
          <form onSubmit={handleSubmit}>
            ...
            <FormInput 
                type='text'
                placeholder='Despacho grande #1A'
              ...
              <FormControl isInvalid={touched.active && errors.active}>
                ...
                <Select 
                  placeholder='Estado del despacho'
                  onChange={handleChange}
                  onBlur={handleBlur}
                  value={values.active}
                  name='active'
