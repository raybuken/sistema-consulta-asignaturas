...
import { getCareersByPage } from "../../helpers/career/careerHelpers"
...
function CareerResults() {
    ...
    function getCareers(pageIndex = 1) {
        getCareersByPage(pageIndex)
            .then(res => res.json())
            .then(data => {
                if(data.ok){
                    setResults(data)
    ...
    return (
        <Container maxWidth={"auto"} padding={"10rem"}>
            <HStack justifyContent={"flex-end"}>
                ...
                {
                    (results.ok && results.data.length > 0) && <Pagination pagination={pagination} updateData={getCareers} />
                }
            </HStack>
            {
